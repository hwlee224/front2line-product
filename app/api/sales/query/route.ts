...
export async function POST(request: Request) {
  ...
  const allStarts = [periods.A.start, periods.B.start, comparisonMode === "3" ? periods.C.start : null].filter(Boolean);
  const allEnds = [periods.A.end, periods.B.end, comparisonMode === "3" ? periods.C.end : null].filter(Boolean);
  ...
  const resultRows = products.map((product) => {
