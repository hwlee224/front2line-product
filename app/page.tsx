...
export default function Page() {
  ...
  async function saveSettings() {
    ...
    await fetch("/api/settings/merge-keywords", {
      method: "POST",
      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-4xl font-semibold tracking-tight">제품별 매출 누적 BI</h1>
            <p className="mt-3 max-w-2xl text-slate-600">한 번 저장한 매출 데이터는 DB에 누적됩니다. 이후에는 하루 파일만 추가 업로드해도 원하는 기간을 자유롭게 비교할 수 있습니다.</p>
          </div>
        ...
        <section className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="rounded-3xl bg-white p-6 shadow-sm">
            ...
            <label className="block cursor-pointer rounded-2xl border border-dashed border-slate-300 p-5 hover:bg-slate-50">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-500">파일명은 반드시 2026년05월05일 형식의 날짜를 포함해야 합니다. 주말 파일 2~3개도 한 번에 업로드 가능합니다.</p>
